...
function getTableDomRows(dom){
    ...
    for(let i=0,len=domHeader.length;i<len;i++){
        ...
        for(let j=0;j<count;j++){
            ...
            tr['index'+j] = {value:text,colspan:colSpan,rowspan:rowSpan, type:isDecimal?"decimal":"string"}
...
function fillTableData({header,body}){
    ...
    for(let rowIndex =0,len =headerCopy.length;rowIndex<len;rowIndex++){
        ...
        for(let i = 0,headerCopyLen = headerCopy[rowIndex].length;i<headerCopyLen;i++){
    ...
    for(let rowIndex =0,len =bodyCopy.length;rowIndex<len;rowIndex++){
        ...
        for(let i = 0,headerCopyLen = bodyCopy[rowIndex].length;i<headerCopyLen;i++){
...
function fillCell(data,rowIndex,colIndex,cell) {
    ...
    }else if(rowspan>1 && colspan===1){ // 行合并
        fillRow(data,rowspan,rowIndex,colIndex,cell)
    }else if(rowspan>1 && colspan>1){ // 列行都合并
...
function fillCol(colspan,row,colIndex,cell){
    ...
    for(let i=1;i<colspan;i++){ // 在当前坐标添加数据，添加colspan-1次
        row.splice(colIndex+1,0,Object.assign(cellcopy,{colspan:1,rowspan:1}))
    }
}
...
function fillRow(data,rowspan,rowIndex,colIndex,cell){
    let cellcopy =Object.assign({},cell);
    for(let i=1;i<rowspan;i++){ // 在当前坐标添加数据，添加rowspan-1次
        data[rowIndex+i].splice(colIndex,0,Object.assign(cellcopy,{colspan:1,rowspan:1}))
    }
}
...
function getDataAndmerges(tableData){
    let excelData=[];
    let merges=[];
    for(let rowIndex=0,rLen = tableData.length;rowIndex<rLen;rowIndex++){//循环表格数据
        let excelRow = [];
            for(let colIndex=0,cLen=tableData[rowIndex].length;colIndex<cLen;colIndex++){ // 循环每一行数据
...
function getMerge(cell,rowIndex,colIndex){
    ...
    let e={} // 最后一个单元格
    if(cell.colspan>1 && cell.rowspan===1){ // 列合并
        e={r:rowIndex,c:colIndex+cell.colspan-1}
    }else if(cell.rowspan>1 && cell.colspan===1){ // 行合并
        e={r:rowIndex+cell.rowspan-1,c:colIndex}
    }else if(cell.rowspan>1 && cell.colspan>1){ // 行列合并
...
function getWs(data,headNum){
    ...
    for (var R = 0; R != data.length; ++R) { // 行
        for (var C = 0; C != data[R].length; ++C) { // 列
            ...
            if(cellObject.type==="decimal"&&cellObject.value==="--")value="-";
            var cell = {v: value};
